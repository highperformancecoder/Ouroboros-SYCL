...
//                                            const QueueDataType element);
  ...
  __dpct_inline__ unsigned int enqueue(const Desc&,const unsigned int position,
	...
	//
        ...
        accessLinked(const unsigned int position);
	...
	// ##############################################################################################################################################
	//
        __dpct_inline__ void setBackPointer(QueueChunk **queue_next_ptr);
	...
	//
        template <typename MemoryManagerType>
        __dpct_inline__ void setOldPointer(MemoryManagerType *memory_manager,
	...
	//
        ...
        locateQueueChunkForPosition(const unsigned int v_position,
                                    const char *message = "");
