...
{
        template <typename T> static T getNextPow2(T n)
        {
		return 1 << (getNextPow2Pow(n));
	}

        template <typename T> static int getNextPow2Pow(T n)
        {
		if ((n & (n - 1)) == 0)
...
struct QueueIndex
{
	...
	static constexpr int smallest_page_factor{AllocationHelper::static_getNextPow2Pow<unsigned int, SMALLEST_PAGE_SIZE>()};
        ...
        static int getQueueIndex(size_t size)
        {
        ...
        static int getPageSizeFromQueueIndex(unsigned int index)
        {
