...
namespace Ouro
{
  ...
  public:
    ...
    void getValues(int &count, int &expected, int &reserved) const
    {
      count = getCount();
    ...
    // Create a new value
    void createValueInternal(int count, int expected, int reserved)
    {
      value = createValueExternal(count, expected, reserved);
    }

    void read(BulkSemaphore &semaphore)
    {
  ...
  public:
    ...
    void wait(const Desc&,int N, uint32_t number_pages_on_chunk,
